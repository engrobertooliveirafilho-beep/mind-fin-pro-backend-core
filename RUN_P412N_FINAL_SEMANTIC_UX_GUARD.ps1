...
if "def p412n_final_semantic_ux_guard" not in s:
    ...
    s=s.replace(marker, insert + "`n`n" + marker, 1)
...
"@ | Set-Content "_p412n_final_semantic_guard_patch.py" -Encoding UTF8

python _p412n_final_semantic_guard_patch.py
...
$msgs=@(
"Oi","Tudo bem?","Quem é você?","Como você está?","Por que está dando erro?",
"Aprofunde","E depois?","Procure o erro principal","Quanto é 4x6",
"deu errado de novo","consegue detalhar mais?","busque pelo problema"
)
...
foreach($m in $msgs){
  ...
  $res += [pscustomobject]@{message=$m;response=$r.Content}
}
