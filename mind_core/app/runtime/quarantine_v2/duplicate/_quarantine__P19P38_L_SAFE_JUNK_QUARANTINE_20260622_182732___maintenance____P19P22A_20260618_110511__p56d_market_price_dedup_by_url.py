...
rows=requests.get(
    f"{u}/rest/v1/p55a_valuation_events?select=id,animal_id,amount,raw_payload,created_at&event_type=eq.P5.6D_MARKET_PRICE&order=created_at.asc&limit=5000",
    headers=h
).json()
...
for r in rows:
    ...
    key=(
        ...
        str(r.get("amount")),
        raw.get("source_url"),
        raw.get("classified_market_event")
    )
    groups[key].append(r)
...
audit={
    "mission":"P5.6D_MARKET_PRICE_DEDUP_BY_SOURCE_URL",
...
open("p56d_market_price_dedup_by_url_audit.json","w",encoding="utf-8").write(json.dumps(audit,ensure_ascii=False,indent=2))
