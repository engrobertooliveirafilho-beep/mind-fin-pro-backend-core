...
from app.runtime.p2091_2100_realtime_intelligence_layer import (
    enforce_paper_only,
...
def test_p2091_2100_intelligence_certification(tmp_path):
    ...
    assert result["readiness"] == "REALTIME_INTELLIGENCE_LAYER_CERTIFIED"
    assert result["mission_status"]["P2100_INTELLIGENCE_CERTIFICATION"] == "PASS"
    ...
    assert len(result["intelligence_outputs"]["ranking"]) >= 3
    assert result["intelligence_outputs"]["top_specialist"] is not None
