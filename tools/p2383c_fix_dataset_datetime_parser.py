...
def simulate(c, prev, nxt, tf):
    ...
    return {
        "entry_time":c["time"],
        "family":"OOS_CANDLE_CONTEXT_PROXY",
        ...
        "mae_r":0,
        "mfe_r":round(abs(rr),6),
        "rr_realized":round(rr,6),
        "profit_factor_proxy":0,
        "expectancy_r_proxy":round(rr,6),
    }
...
def run(dataset, invalid_prev, outdir):
    safety()
    out=Path(outdir)
    ...
    for tf,file in TIMEFRAME_FILES.items():
        ...
        tf_results.append({
            ...
            "invalid_candles":len(raw)-len(valid),
        ...
        write_csv(out/f"de40_oos_fixed_signals_{tf}.csv",signals,list(signals[0].keys()) if signals else ["entry_time"])

    global_m=metrics(all_signals)
    promoted=[r for r in tf_results if r["passed"]]
    rejected=[r for r in tf_results if not r["passed"]]

    write_csv(out/"de40_oos_fixed_dataset_audit.csv",dataset_audit,list(dataset_audit[0].keys()))
    write_csv(out/"de40_oos_fixed_all_signals.csv",all_signals,list(all_signals[0].keys()) if all_signals else ["entry_time"])
    write_csv(out/"de40_oos_fixed_timeframe_results.csv",tf_results,list(tf_results[0].keys()))
    write_csv(out/"de40_oos_fixed_promoted.csv",promoted,list(tf_results[0].keys()))
    write_csv(out/"de40_oos_fixed_rejected.csv",rejected,list(tf_results[0].keys()))

    for dim in ["regime","session","footprint","lifecycle","timeframe"]:
        rows=stability(all_signals,dim)
        write_csv(out/f"de40_oos_fixed_{dim}_stability.csv",rows,list(rows[0].keys()) if rows else ["dimension"])

    summary={
        "mission":"P2383C_FIX_DATASET_DATETIME_PARSER",
        "mode":MODE,
        "real_orders":REAL_ORDERS,
        "ftmo_real":FTMO_REAL,
        "dataset":str(dataset),
        "invalid_previous_p2383c":str(invalid_prev),
        "output":str(out),
        "fix":"Added parser for MetaTrader datetime format %Y.%m.%d %H:%M:%S.",
        "method":"80/20 chronological split on certified DE40 raw candles; last 20 percent used as OOS proxy.",
        "global_metrics":global_m,
        "timeframes_tested":len(tf_results),
        "promoted":len(promoted),
        "rejected":len(rejected),
        "certification":"OOS_PROXY_PROMOTED" if len(promoted)>0 else "OOS_PROXY_NOT_PROMOTED",
...
if __name__=="__main__":
    ...
    p.add_argument("--invalid-prev",required=True)
    p.add_argument("--out",required=True)
    a=p.parse_args()
    print(json.dumps(run(a.dataset,a.invalid_prev,a.out),indent=2,ensure_ascii=False))
