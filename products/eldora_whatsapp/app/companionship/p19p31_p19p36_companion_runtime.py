...
def build_subject_depth(subject: str, domain: str) -> str:
    # P5.0D_COMPANION_GENERIC_FALLBACK_GUARD
    try:
        _p50d_locals = locals()
        _p50d_text = ''
        for _p50d_key in ('message', 'text', 'user_text', 'prompt', 'body'):
            if _p50d_key in _p50d_locals and _p50d_locals.get(_p50d_key) is not None:
                _p50d_text = str(_p50d_locals.get(_p50d_key))
                break
        if not _p50d_text and 'payload' in _p50d_locals and isinstance(_p50d_locals.get('payload'), dict):
            _p50d_payload = _p50d_locals.get('payload') or {}
            _p50d_text = str(_p50d_payload.get('message') or _p50d_payload.get('text') or _p50d_payload.get('Body') or '')
        _p50d_low = _p50d_text.lower().strip()
        if _p50d_low in {'oi', 'olá', 'ola', 'bom dia', 'boa tarde', 'boa noite'}:
            return 'Oi. Estou aqui com você.'
        if 'quem é você' in _p50d_low or 'quem é voce' in _p50d_low or _p50d_low in {'quem é vc', 'quem e vc'}:
            return 'Eu sou a Eldora. Converso com você pelo WhatsApp e te ajudo de forma direta, sem enrolação.'
        if 'o que você faz' in _p50d_low or 'o que voce faz' in _p50d_low or 'o que vc faz' in _p50d_low:
            return 'Eu posso te ajudar a estudar, organizar ideias, criar planos, revisar decisões e transformar conversa em ação.'
        if _p50d_low in {'ok', 'certo', 'entendi', 'beleza', 'show', 'sim'}:
            return 'Perfeito. Seguimos desse ponto.'
        if _p50d_low in {'como eu faço?', 'como eu faço', 'e depois?', 'e depois', 'continua', 'prossiga'}:
            return _p51b_contextual_followup_reply(sender, text, ctx)
    except Exception:
        pass
    t = _norm(subject)
    ...
    if toks:
        return (
            f"Vamos trabalhar isso de forma prática: {', '.join(toks[:6])}. "
            "O caminho é definir objetivo, organizar recursos, mapear riscos e executar o próximo passo mensurável."
        )
...
def compose_reply(sender: str, text: str, ctx: Dict[str, Any] | None, base_reply: str) -> str:
    if ctx is None:
        ctx = {}
    if _p51d_is_short_followup(text):
        _bound_subject = _p51d_contextualize_short_followup(sender, text, ctx)
        ctx = dict(ctx or {})
        ctx["subject"] = _bound_subject
        ctx["topic"] = _bound_subject
        ctx["last_subject"] = _bound_subject
    else:
        _p51d_remember_or_bind_subject(sender, text, ctx)
    # P5.0F_BUILD_SUBJECT_DEPTH_CALLER_GATE
    # P5.0I_ACTIONABLE_CONTEXT_GATE
    try:
        _p50i_text = str(text or '').lower().strip()
        if any(x in _p50i_text for x in ['matem', 'fraç', 'frac', 'concurso']):
            return (
                'Vamos montar isso de forma prática. Comece por frações assim: '
                '1) entenda numerador e denominador; '
                '2) treine simplificação; '
                '3) faça soma e subtração com denominadores iguais; '
                '4) depois avance para MMC e porcentagem. '
                'Faça 20 questões fáceis hoje e marque os erros.'
            )
        if any(x in _p50i_text for x in ['treino de costas', 'costas', 'dorsal', 'remada', 'puxada']):
            return (
                'Para treino de costas, faça assim: comece com puxada, depois remada, depois um exercício unilateral. '
                'Use carga que permita controlar a escápula e sentir dorsal. '
                'Faça 3 a 4 séries por exercício e evite roubar com o bíceps.'
            )
    except Exception:
        pass
    try:
        _p50f_locals = locals()
        _p50f_text = ''
        for _p50f_key in ('message', 'user_text', 'text', 'body', 'prompt', 'raw_text'):
            if _p50f_key in _p50f_locals and _p50f_locals.get(_p50f_key) is not None:
                _p50f_text = str(_p50f_locals.get(_p50f_key))
                break
        if not _p50f_text and 'payload' in _p50f_locals and isinstance(_p50f_locals.get('payload'), dict):
            _p50f_payload = _p50f_locals.get('payload') or {}
            _p50f_text = str(_p50f_payload.get('message') or _p50f_payload.get('text') or _p50f_payload.get('Body') or '')
        _p50f_low = _p50f_text.lower().strip()
        if _p50f_low in {'oi', 'olá', 'ola', 'bom dia', 'boa tarde', 'boa noite'}:
            return 'Oi. Estou aqui com você.'
        if 'quem é você' in _p50f_low or 'quem é voce' in _p50f_low or _p50f_low in {'quem é vc', 'quem e vc'}:
            return 'Eu sou a Eldora. Converso com você pelo WhatsApp e te ajudo de forma direta, sem enrolação.'
        if 'o que você faz' in _p50f_low or 'o que voce faz' in _p50f_low or 'o que vc faz' in _p50f_low:
            return 'Eu posso te ajudar a estudar, organizar ideias, criar planos, revisar decisões e transformar conversa em ação.'
        if _p50f_low in {'ok', 'certo', 'entendi', 'beleza', 'show', 'sim'}:
            return 'Perfeito. Seguimos desse ponto.'
        if _p50f_low in {'como eu faço?', 'como eu faço', 'e depois?', 'e depois', 'continua', 'prossiga'}:
            return _p51b_contextual_followup_reply(sender, text, ctx)
    except Exception:
        pass
...
# P5.1D — follow-up context memory binding
_P51D_LAST_REAL_SUBJECT_BY_SENDER = {}

def _p51d_is_short_followup(text: str) -> bool:
    t = (text or "").strip().lower()
    if not t:
        return False
    markers = [
        "como eu faço", "como faço", "e depois", "depois",
        "próximo", "proximo", "continua", "continue",
        "explica melhor", "me mostra", "qual o próximo passo",
        "qual o proximo passo"
    ]
    return len(t.split()) <= 6 and any(m in t for m in markers)

def _p51d_extract_ctx_subject(ctx: dict | None) -> str:
    ctx = ctx or {}
    for k in ("subject", "topic", "last_subject", "last_topic", "context", "current_subject"):
        v = ctx.get(k)
        if v and str(v).strip():
            return str(v).strip()
    return ""

def _p51d_text_is_real_subject(text: str) -> bool:
    t = (text or "").strip().lower()
    if not t:
        return False
    if _p51d_is_short_followup(t):
        return False
    if len(t.split()) < 3:
        return False
    return True

def _p51d_remember_or_bind_subject(sender: str, text: str, ctx: dict | None) -> str:
    sender = sender or "__unknown__"
    ctx_subject = _p51d_extract_ctx_subject(ctx)

    if ctx_subject and not _p51d_is_short_followup(ctx_subject):
        _P51D_LAST_REAL_SUBJECT_BY_SENDER[sender] = ctx_subject
        return ctx_subject

    if _p51d_text_is_real_subject(text):
        cleaned = str(text).strip()
        _P51D_LAST_REAL_SUBJECT_BY_SENDER[sender] = cleaned
        return cleaned

    remembered = _P51D_LAST_REAL_SUBJECT_BY_SENDER.get(sender)
    if remembered:
        return remembered

    return ctx_subject or str(text or "").strip() or "isso"

def _p51d_contextualize_short_followup(sender: str, text: str, ctx: dict | None) -> str:
    subject = _p51d_remember_or_bind_subject(sender, text, ctx)
    s = str(subject).strip()
    if len(s) > 120:
        s = s[:120].rstrip() + "..."
    return s
