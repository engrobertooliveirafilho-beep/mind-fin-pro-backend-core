...
PREV = OUT / "p1505f_mt5_priority_backtest_results.json"
RESULTS = OUT / "p1505k_remaining_mt5_backtest_results.json"
REPORT = OUT / "p1505k_remaining_mt5_backtest_report.json"

BATCH_LIMIT = 1000

TARGET_ASSETS = {"USDCAD","USDJPY","XAUUSD","GBPUSD"}
...
def job_key(j):
    return "|".join([
        str(j.get("dataset")),
        str(j.get("family")),
        json.dumps(j.get("params"), sort_keys=True, default=str)
    ])
...
prev = load_json(PREV, [])
tested = set(job_key(j) for j in prev)

remaining = [
    j for j in queue
    if j.get("source") == "MT5"
    and j.get("status") == "QUEUED"
    and j.get("asset") in TARGET_ASSETS
    and job_key(j) not in tested
]
...
for r in results:
    ...
    if r.get("approved_backtest"):
        approved_counts[a] = approved_counts.get(a, 0) + 1

report = {
    "STATUS": "P1505K_REMAINING_MT5_ASSETS_BACKTEST_COMPLETED",
    "TARGET_ASSETS": sorted(list(TARGET_ASSETS)),
    "REMAINING_AVAILABLE": len(remaining),
    "JOBS_INPUT": len(batch),
    "BACKTESTED": len([r for r in results if r.get("status") == "BACKTESTED"]),
    "APPROVED_BACKTESTS": approved,
    "ERRORS": errors,
    "ASSET_TESTED_COUNTS": asset_counts,
    "ASSET_APPROVED_COUNTS": approved_counts,
    "RESULTS_FILE": str(RESULTS),
    "NEXT": "P1505L_WALK_FORWARD_MONTE_CARLO_REMAINING_MT5",
    "REAL_ORDERS": "FORBIDDEN",
    "FTMO_REAL": "FORBIDDEN",
    "MT5_REAL": "FORBIDDEN",
