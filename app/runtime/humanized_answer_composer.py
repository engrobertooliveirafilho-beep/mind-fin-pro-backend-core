...
def humanized_answer(message: str, answer: str, ctx: dict | None = None) -> str:
    ...
    if ctx.get("last_domain") == "vehicle_buying" and subject:
        ...
        if any(x in msg for x in ["vale a pena"]):
            return f"Vale a pena sim, mas só se a {subject} estiver inteira. Eu compraria com laudo, teste frio e histórico de revisão. Se o dono não prova manutenção, eu negociaria forte ou deixaria passar."
