...
def run_cognitive_pipeline(user_id: str, message: str) -> dict:
    ...
    if any(x in msg_l for x in ["tudo bem", "como ta", "como t├í", "ta bem", "t├í bem"]):
        return {"answer": "Est├í funcionando bem, de forma natural. E com voc├¬?", "intent": "smalltalk"}
    ...
    if any(x in msg_l for x in [
        ...
        "cad├¬ a resposta",
        "onde ta a resposta",
        "onde est├í a resposta"
    ]):
        answer = "Roberto, resposta direta: o problema atual n├úo ├® infraestrutura. ├ë continuidade conversacional."
        save_message(user_id, "assistant", answer)
        return {
            "answer": answer,
