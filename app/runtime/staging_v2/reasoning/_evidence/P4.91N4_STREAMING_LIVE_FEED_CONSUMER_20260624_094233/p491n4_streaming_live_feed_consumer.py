...
BACKEND = Path(os.environ["P491N4_BACKEND"])
EVIDENCE = Path(os.environ["P491N4_EVIDENCE"])

FEEDS = [
    ("runtime", Path(os.environ["P491N4_RUNTIME"])),
    ("code_matrix", Path(os.environ["P491N4_CODE_MATRIX"])),
    ("capability", Path(os.environ["P491N4_CAPABILITY"])),
    ("mind_items", Path(os.environ["P491N4_MIND_ITEMS"])),
]
...
LEDGER = EVIDENCE / "processed_ledger_p491n4.jsonl"
...
def resolve_path(raw):
    ...
    hits = [h for h in hits if h.exists() and not blocked(h)]
...
def stream_jsonl(feed_name, path, start_line):
    ...
    with open(path, "r", encoding="utf-8", errors="replace") as f:
        for i, line in enumerate(f):
            ...
            if not raw:
                continue
            if not allowed(raw) or blocked(raw):
                continue
...
def collect_streaming(cursor):
    rows = []
    feed_status = []

    for feed_name, path in FEEDS:
        key = str(path)
        start_line = int(cursor.get(key, 0))
        ...
        try:
            if path.suffix.lower() == ".jsonl":
                got, last_line, scanned = stream_jsonl(feed_name, path, start_line)
            else:
                got, last_line, scanned = stream_csv(feed_name, path, start_line)

            cursor[key] = last_line
            rows.extend(got)
            feed_status.append({
                "feed":key,
                "exists":True,
...
def main():
    ...
    while True:
        cycle += 1
        rows, feed_status, cursor = collect_streaming(cursor)
        save_cursor(cursor)
