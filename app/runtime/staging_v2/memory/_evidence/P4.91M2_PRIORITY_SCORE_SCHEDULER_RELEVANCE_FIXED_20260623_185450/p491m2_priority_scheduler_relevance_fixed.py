...
MISSION="P4.91M2_PRIORITY_SCORE_SCHEDULER_RELEVANCE_FIXED"
ROOT=Path(r"_evidence\P4.91M2_PRIORITY_SCORE_SCHEDULER_RELEVANCE_FIXED_20260623_185450")
...
CERT=ROOT/"P4.91M2_CERTIFICATION.txt"
...
def capability_relevance(path,categories,ideas):
    p=(path or "").lower()
...
def is_vendor(path):
    p=(path or "").lower()
    return any(b in p for b in BAD_PATTERNS)
...
def load_matrix():
    ...
    with open(MATRIX,encoding="utf-8-sig",newline="") as f:
        for r in csv.DictReader(f):
            ...
            relevance=capability_relevance(path,r.get("categories",""),r.get("ideas",""))

            if score < 3 and code_score < 3 and relevance < 100:
                continue

            r["_score_int"]=score
            r["_code_score_int"]=code_score
            r["_relevance"]=relevance
            r["_is_vendor"]=is_vendor(path)
...
def pick_next(rows,done):
    ...
    if vendor_candidates:
        vendor_candidates.sort(key=lambda x:(-x["_score_int"],x.get("path","")))
        return vendor_candidates[0]
...
def print_monitor():
    ...
    print("")
    print("="*110)
    print("P4.91M2 PRIORITY SCORE SCHEDULER - RELEVANCE FIXED")
    print("="*110)
...
while state["processed"] < MAX_TOTAL:
    ...
    if item["_is_vendor"]:
        rec={
            ...
            "final_status":"ARCHIVE_ONLY",
            "next_action":"IGNORE_VENDOR_CODE",
            "integration_priority":"P99",
            "processed_once":True,
        ...
        update_counts(score,"ARCHIVE_ONLY")
        state["last_status"]="ARCHIVE_ONLY"
        time.sleep(1)
        continue
...
summary={
    ...
    "ledger":str(LEDGER),
    "results":str(RESULTS),
    "scheduler_rule":"SCORE_5_FIRST_WITH_CAPABILITY_RELEVANCE_FILTER_VENDOR_ARCHIVE",
    "original_modified":False,
    "delete":"FORBIDDEN",
    "move_original":"FORBIDDEN",
    "modify_original":"FORBIDDEN",
    "certification":"PRIORITY_SCORE_SCHEDULER_RELEVANCE_FIXED_CERTIFIED"
}
