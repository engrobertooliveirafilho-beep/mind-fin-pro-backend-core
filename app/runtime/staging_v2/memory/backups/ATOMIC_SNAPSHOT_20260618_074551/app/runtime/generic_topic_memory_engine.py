...
def expand_followup(message: str, ctx: dict) -> str:
    if is_followup(message) and ctx.get("last_subject"):
        ...
        if domain=="vehicle_buying" and any(x in message.lower() for x in ["manutenção","manutencao","revisão","revisao"]):
            return f"{message}. Contexto obrigatório: responder sobre {ctx['last_subject']}. Não invente km, preço ou datas. Entregue checklist qualitativo: motor, transmissão, suspensão, freios, pneus, eletrônica, histórico de manutenção, disponibilidade de peças e riscos de compra."
