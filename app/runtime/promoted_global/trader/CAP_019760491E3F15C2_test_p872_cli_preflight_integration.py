...
def test_cli_blocks_preflight_missing_folder(tmp_path):
    cfg=tmp_path/"ftmo.json"; save_default_ftmo_config(str(cfg))
    r=run_paper_research_cli([
        "--data-folder",str(tmp_path/"missing"),
        "--symbol","TEST",
        "--timeframe","1m",
    ...
    assert r["decision"]=="CLI_BLOCKED_PREFLIGHT"
    assert r["production"]=="BLOCKED"

def test_cli_runs_after_preflight_ok(tmp_path):
