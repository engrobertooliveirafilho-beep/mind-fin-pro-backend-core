...
from app.eldora.core.redis_diagnostics import redis_diagnostics

router = APIRouter(
    prefix="/eldora/true-distributed",
    tags=["eldora-true-distributed"]
)
...
@router.get("/redis/diagnostics")
async def diagnostics():
    return redis_diagnostics()

@router.post("/worker/loop")
async def worker_loop(
    worker_name: str = "worker_alpha",
    cycles: int = 1
):
