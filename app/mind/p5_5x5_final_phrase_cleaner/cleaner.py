...
BAD_STARTS = ["the ", "to ", "from ", "for ", "buy ", "buyer ", "seller ", "professional "]
BAD_TERMS = ["professional bull riders", "futurity champion", "female", "prices", "appointment"]

def bad_final_phrase(name):
    n = re.sub(r"\s+", " ", str(name or "").strip().lower())
    if any(n.startswith(x) for x in BAD_STARTS):
        return True
    if any(x in n for x in BAD_TERMS):
        return True
    return False

class FinalPhraseCleaner:
    ...
    def run(self):
        bad=[a for a in self.animals() if bad_final_phrase(a.get("official_name"))]
        deleted_edges=0
        deleted_animals=0
        for a in bad:
        ...
        return {"status":"P5.5X5_FINAL_PHRASE_CLEANER_DONE","bad_detected":len(bad),"animals_deleted":deleted_animals,"edges_deleted":deleted_edges,"deleted_names":[a["official_name"] for a in bad],"next_action":"RECALCULATE_P5.5X_GRAPH"}
