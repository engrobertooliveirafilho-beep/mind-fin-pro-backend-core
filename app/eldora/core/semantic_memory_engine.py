...
def insert_embedding(sender_id: str, text: str, metadata: dict | None = None) -> dict:
    if not text or not text.strip():
        return {"status": "skipped", "stored": False, "reason": "empty_text", "source": "pgvector"}
    if not _database_url():
        return _db_unavailable()

    _ensure_schema()
    embedding = cached_embed(EmbeddingProvider(), text)
    if not embedding:
    ...
    with psycopg2.connect(_database_url(), connect_timeout=5) as conn:
        with conn.cursor(cursor_factory=psycopg2.extras.RealDictCursor) as cur:
            cur.execute("""
            insert into neura_embeddings(sender_id, message, metadata, embedding)
    ...
    return {"status": "ok", "stored": True, "source": "pgvector", "id": str(row["id"]), "sender_id": row["sender_id"]}
...
def similarity_search(sender_id: str, query: str, top_k: int = 3) -> dict:
    return {"status": "ok", "query": query, "results": semantic_search(sender_id or "default", query, top_k)}
...
def store_memory(text: str, metadata: dict | None = None):
    meta = metadata or {}
    sender_id = meta.get("sender_id") or meta.get("from") or "default"
    return insert_embedding(sender_id, text, meta)


def retrieve_memory(query: str, top_k: int = 3):
    return similarity_search("pytest_semantic", query, top_k)
